...
### Scheduling Tasks for AppService
- To run scheduled (cron jobs) tasks, there are 3 options:
    - **AppService WebJobs** (*useful for running scheduled/continuous scripts that influence app content*)
    - **Azure Functions** (*useful for scalable serverless applications*)
    - **Azure Automation** (*way beyond app level usage, only for infrastructure tasks*)

- App hosted in AppService makes it easier to use AppService WebJobs with zero setup and endpoint handling.
- Process is simple:
    1. Prepare the script
    2. Based on the app instance OS (current instance, Linux), use either `run.cmd` or `run.sh` and provide instructions.
    3. Zip the files and upload them, either through portal or through CLI
        - Before upload check whether there are any settings to be adjusted for adding a WebJob (had to change settings to add WebJobs under Linux instance)
    4. Schedule the WebJob and test run to debug errors.

- Unfortunately, if the script generates an output file that needs to be used in the application logic, then this would be a huuuuge diversion!
- Enter, **Azure Functions**

### Schedule and Save Files using Azure Functions
- Serverless functions are used for triggering functions either as a cron job or as response to specific events triggered by the application logic.
- Files generated by Functions can be stored in **Azure Blob Storage**
- **Azure Blob Storage** SDK can be used to move the generated files from **Functions** -> **AppService**
- Process is a WIP. Will update notes once completed...
